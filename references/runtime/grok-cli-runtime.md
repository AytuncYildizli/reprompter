...
This document is additive. It does not modify any behavior for users on Claude Code, Codex CLI, or OpenClaw. Grok CLI users (or anyone whose toolset includes `spawn_subagent`, `run_command`, `ask_user_question`, and `todo_write`) should read this file when the SKILL.md Grok Support section activates Option F.