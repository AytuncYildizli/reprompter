...
- `--sandbox workspace-write` permits writes to the workspace and `/tmp`, which workers need to produce `/tmp/rpt-{taskname}-{agent}.md` artifacts. `--full-auto` would also work here — in `codex exec` mode it selects workspace-write and exec keeps approval at `never` regardless of the alias's interactive-mode default — but we prefer the explicit flag for readability. Avoid `--full-auto` only if you also invoke Codex interactively from the same script, where the alias does set `-a on-request` and would block a backgrounded child.
