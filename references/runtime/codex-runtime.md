...
- `--sandbox workspace-write` permits writes to the workspace and `/tmp`, which workers need to produce `/tmp/rpt-{taskname}-{agent}.md` artifacts. Prefer this explicit form over `--full-auto` in background runs: Codex's help text defines `--full-auto` as a workspace-write + `on-request` approval preset, and reports on exact exec-mode behavior have been inconsistent across releases. With `--sandbox <mode>` and no `-a` flag, `codex exec` reliably keeps its headless default of approval = `never`, which is what background `wait` fan-outs require. If you must use `--full-auto`, add `-a never` explicitly.
