...
|---|---|---|
| Ends when | the done-list is complete | you stop it — it stalls after each long turn until you resend, and the hard ceiling is the session's context window |
...
- **On that plan, context filled before quota did** — 98% of the session window while the usage quota held. On a smaller plan quota will bite first. Either way, when context fills the session compacts and loses what was already tried, which is where quality tends to plateau.
- **Progress was not smooth.** Three blockers looked permanent at hour 8 and every performance budget was met by hour 20. Plateaus sometimes break after the model reframes the problem — but decide whether to continue on a measured gate, not on elapsed hope.
- **Keep the best measured state, not the last commit.** Mid-run refactors caused real regressions there: quality dipped after a rewrite before exceeding its previous best. Tag or note whichever state your checks actually passed on.
...
Rule: when a builder claims done and the artifact still looks wrong, suspect the check or a missing checklist item first — the builder is usually reporting honestly against what it was actually asked to satisfy.
