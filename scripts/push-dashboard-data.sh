...
upsert_key() {
  local key="$1"
  local json_data="$2"
  PGPASSWORD="REDACTED_NEON_PASSWORD" psql "$DB_URL" -q -c "
...
echo ""
echo "📋 Verification:"
PGPASSWORD="REDACTED_NEON_PASSWORD" psql "$DB_URL" -q -c "SELECT key, updated_at, pg_column_size(data) as bytes FROM system_status ORDER BY key;" 2>/dev/null
