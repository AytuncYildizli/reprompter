...
Hermes Agent can run the same local Ambient Prompt Gate before model calls. Use a RePrompter git clone or copied checkout that includes `scripts/` (Hermes skill installs ship no `scripts/` helpers), then document this shell hook in `~/.hermes/config.yaml`:
