...
If ANY of the following signals are present, SKIP Quick Mode and go directly to interview:
...
Broad-scope noun enforcement: Count the number of distinct systems/modules implied by broad-scope nouns (system, module, suite, platform, pipeline, dashboard, management). If count >= 1 AND the prompt does not name a single, specific identifier — force interview.
