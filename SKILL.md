...
- **Grok delivery target requires headless-relay v2.0.0+.** headless-relay only isolates Grok
  fail-closed (non-git working dir) from v2.0.0 onward (**v2.0.3+ recommended: it also runs Grok
  with a clean `GROK_HOME`, so your global `~/.grok` rules do not leak into the model turn**);
  this repo's documented relay minimum is v1.3.1+, which is fine for the other targets. So if you
  cannot confirm the installed relay is v2.0.0+ (e.g. its SKILL.md `metadata.version`), do NOT
  offer Grok as a delivery target — an older relay would run Grok in the caller's repo and upload
  it. The other targets are unaffected.
