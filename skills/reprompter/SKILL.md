...
**Ultracode:** when ultracode is on, the emitted script defaults to the thorough body — adversarial / perspective-diverse verify per finding, a completeness critic, and budget-scaled fleets; a lean off-ramp (`REPROMPTER_ULTRACODE=0` / `--no-ultracode`) keeps trivial reprompts cheap. Compiler: `the root repository workflow compiler`.
