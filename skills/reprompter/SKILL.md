...
### Version self-check
RePrompter is distributed copy-based (no package manager tracks the installed version), so it can't auto-update — but it can tell the user when their copy is stale. Unless `REPROMPTER_VERSION_CHECK=0`, on the **first** reprompter invocation in a session run `the root repository version-check helper` once and, if it prints an update notice, surface that notice to the user before continuing. Never block on it.
