...
| Situation | Pick |
|---|---|
| Agents coordinate from the same task brief; one synthesis output expected | D1 |
...
```toml
# Codex agents directory/rpt_audit_explorer.toml
...
- `$PROMPT_TEXT` is the full reprompted prompt from Phase 2. Pass via `a pre-read prompt string`; do not pipe via stdin (Codex treats stdin as a conversation continuation in some modes).
