...
> v2.0.3+ additionally uses a clean `GROK_HOME` and denies tools, so your global `~/.grok` rules
...
Optional configuration in `Grok config file`:
...
- When using custom personas or roles, the definition must exist in `Grok personas directory/*.toml` or `~/.grok/roles/*.toml`, otherwise spawn fails (fail-closed).
...
- `~/.grok/docs/user-guide/15-subagent guide`
